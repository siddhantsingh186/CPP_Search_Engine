...
void openURL(const string& url_str) {
    string command = "start " + url_str; // Use "start" command to open URL in the default browser
...
long Solutions::Life_solution(string wordSearch, int pushKey) {
    ...
    ifstream getFile("Generic_Problem.txt"); // Update the path to your Generic_Problem.txt file
...
void Solutions::subSearch(int subSearchArray, int noOfWords, string searchedString) {
    ...
    if (subSearchArray <= noOfWords && noOfWords > 0) {
        try {
            ...
            ifstream soln("Generic_Solution.txt"); // Update the path to your Generic_Solution.txt file
            if (soln.is_open()) {
        ...
        out.open("Generic_Problem.txt", std::ios::app); // Update the path to your Generic_Problem.txt file
